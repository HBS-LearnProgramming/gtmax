...
// Allow our REST namespace when another plugin restricts the REST API to authenticated users.
// Our endpoints use token-based auth (permission_callback), so we must get past the global check.
add_filter('rest_authentication_errors', 'geoguru_allow_rest_namespace_when_restricted', 20, 1);
...
/**
 * When another plugin restricts the REST API to authenticated users only (rest_api_authentication_required),
 * allow requests to our namespace through so our token-based permission_callback can run.
 *
 * @param WP_Error|null|true $result Result from previous filters.
 * @return WP_Error|null|true Pass through, or null to allow our namespace when globally restricted.
 */
function geoguru_allow_rest_namespace_when_restricted($result) {
    if (!is_wp_error($result) || $result->get_error_code() !== 'rest_api_authentication_required') {
        return $result;
    }
    $uri = isset($_SERVER['REQUEST_URI']) ? sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI'])) : '';
    if (strpos($uri, '/wp-json/geoguru/v1/') === false && strpos($uri, '/wp-json/geoguru-api/') === false) {
        return $result;
    }
    return null;
}
