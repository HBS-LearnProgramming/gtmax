...
/**
 * Whether the JSON error code indicates WordPress blocked REST before route permission_callback ran.
 *
 * @param string $code REST JSON "code" field.
 * @return bool
 */
function geoguru_diagnostic_is_global_rest_auth_error_code($code) {
    $blocked = array(
        'rest_api_authentication_required',
        'rest_not_logged_in',
    );
    return $code !== '' && in_array($code, $blocked, true);
}
...
function geoguru_diagnostic_check_rest_namespace_not_globally_locked() {
    ...
    $http = (int) wp_remote_retrieve_response_code($response);
    $err = geoguru_diagnostic_rest_json_error_code($response);
    if (geoguru_diagnostic_is_global_rest_auth_error_code($err)) {
        return array(
            'pass' => false,
            'message' => sprintf(
                /* translators: %s: REST error code */
                __('Bad: anonymous request returned "%s". A plugin or theme is forcing WordPress login for all REST traffic before GeoGuru’s routes run. GeoGuru expects its namespace to stay reachable so token-based auth can run (see rest_authentication_errors).', 'lovedbyai-seo-for-llms-and-ai-search'),
                $err
            ),
        );
    }
    $body = wp_remote_retrieve_body($response);
    if ($err === '' && ($http === 401 || $http === 403) && strpos($body, 'rest_api_authentication_required') !== false) {
        return array(
            'pass' => false,
            'message' => __('Bad: response looks like rest_api_authentication_required (HTML or non-JSON). Site-wide REST authentication is blocking this namespace.', 'lovedbyai-seo-for-llms-and-ai-search'),
        );
    }
    if ($http === 401 || $http === 403) {
        return array(
            'pass' => true,
            'message' => sprintf(
                /* translators: 1: HTTP status, 2: JSON error code or "rest_forbidden" hint */
...
function geoguru_diagnostic_check_rest_public_llm_event_not_globally_locked() {
    ...
    if (geoguru_diagnostic_is_global_rest_auth_error_code($err)) {
        return array(
            ...
            'message' => sprintf(
                /* translators: %s: REST error code */
                __('Bad: public tracking endpoint returned "%s". Anonymous clients cannot reach it; site-wide REST login requirements may be blocking all /wp-json/ routes.', 'lovedbyai-seo-for-llms-and-ai-search'),
                $err
            ),
        );
    }
    ...
    if ($err === '' && ($http === 401 || $http === 403) && strpos($body, 'rest_api_authentication_required') !== false) {
        return array(
            'pass' => false,
            'message' => __('Bad: public endpoint response suggests rest_api_authentication_required. Check REST authentication filters and server rules.', 'lovedbyai-seo-for-llms-and-ai-search'),
        );
    }
    if ($http === 401 || $http === 403) {
        return array(
            ...
            'message' => sprintf(
                ...
                __('Bad: public endpoint returned HTTP %1$d (code: %2$s). It should not require WordPress login; expect 400 validation or 5xx upstream, not 401/403.', 'lovedbyai-seo-for-llms-and-ai-search'),
                $http,
                $err !== '' ? $err : __('(unparsed)', 'lovedbyai-seo-for-llms-and-ai-search')
            ),
        );
