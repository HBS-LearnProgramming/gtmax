...
if (!class_exists('GeoGuru_RequestsInterceptor')) {
    class GeoGuru_RequestsInterceptor {
        ...
        /**
         * Enqueue client-side script for LLM source event tracking (proxy URL only; no secret).
         * Script sends raw url, referer, utm_source and user_agent to the proxy; edge function does allowlist validation.
         */
        public function enqueue_llm_tracking_script() {
            ...
            $inline = sprintf(
                "window.geoguru_llm_tracking={proxyUrl:%s};",
                wp_json_encode($proxy_url)
            );
            $inline .= "(function(){var c=window.geoguru_llm_tracking;if(!c||!c.proxyUrl)return;
var payload=JSON.stringify({url:location.href,referer:document.referrer||'',utm_source:(new URLSearchParams(window.location.search)).get('utm_source')||'',user_agent:navigator.userAgent||''});
if(typeof fetch==='function'&&typeof Request!=='undefined'){fetch(c.proxyUrl,{method:'POST',body:payload,keepalive:true,headers:{'Content-Type':'application/json'}}).catch(function(){});}else if(typeof navigator.sendBeacon==='function'){var blob=new Blob([payload],{type:'application/json'});navigator.sendBeacon(c.proxyUrl,blob);}
})();";

            wp_add_inline_script('geoguru-llm-tracking', $inline);
        }
